...
export default function GateExit() {
  ...
  const loadData = async () => {
    try {
      const logs = await activityService.getSystemActivity(20);
      setExitLog(
        logs.filter((log) => log.action === "EXIT" || log.type === "EXIT"),
      );
    } catch (error) {
      console.error("Failed to load exit log", error);
    }
  };
    ...
    try {
      ...
      const items = [
        ...sessions.map((s) => ({
          id: s.card,
          plate: s.vehicle,
          typeDesc: s.vehicleType,
          isTemp: false,
          isMoto: s.vehicleType?.toLowerCase().includes("máy"),
          label: `${s.card} • ${s.vehicle} (${s.vehicleType})`,
        })),
        ...tickets.map((t) => ({
          id: t.id,
          plate: t.plate,
          typeDesc: "Khách vãng lai",
          isTemp: true,
          isMoto: false,
          label: `${t.id} • ${t.plate} (Khách)`,
        })),
      ];
  ...
  const handleClose = () => {
    ...
    setExitInput("");
    setScanResult(null);
    setStatusMessage("Chưa quét thẻ");
    setDemoState("normal");
    loadActiveItems();
  };
  ...
  const handleExitScan = async () => {
    ...
    if (input.startsWith("TEMP-")) {
      ...
      setStatusMessage(
        `Thẻ tạm ${input} · Xe ${ticket.plate} · Phí: ${fee.toLocaleString("vi-VN")} VNĐ — Chờ thanh toán.`,
      );
      setIsProcessing(false);
      return;
    }
    ...
    // Branch: regular card
    const exitResult = await parkingService.processExit(input);
    ...
    if (!isGuest) {
      ...
      const memberResult = await parkingService.completeMemberExit(
        session.id,
        session.card,
      );
      setScanResult({ ...memberResult.session, cardInfo });
      setDemoState("open");
      setIsBarrierOpen(true);
      setStatusMessage(
        `Xe ${session.vehicle} đã ra. Phí ${fee.toLocaleString("vi-VN")} VNĐ đã ghi nợ.`,
      );
      loadData();
    } else {
      // Guest card: wait for payment — barrier stays LOCKED
      setScanResult({ ...session, fee, cardInfo });
      setPendingExitData({ type: "card", session, fee, cardInfo });
      setIsWaitingPayment(true);
      setStatusMessage(
        `Khách vãng lai · Xe ${session.vehicle} · Phí: ${fee.toLocaleString("vi-VN")} VNĐ — Chờ thanh toán.`,
      );
    }
  ...
  const handleConfirmPayment = async () => {
    ...
    if (pendingExitData.type === "ticket") {
      const result = await parkingService.completeGuestTicketExit(
        pendingExitData.ticket.id,
      );
      if (result.success) {
      ...
      const result = await parkingService.completeGuestExit(
        pendingExitData.session.id,
      );
      if (result.success) {
        setScanResult({
          ...result.session,
          cardInfo: pendingExitData.cardInfo,
        });
        setDemoState("open");
        setIsBarrierOpen(true);
        setIsWaitingPayment(false);
        setPendingExitData(null);
        setStatusMessage(
          `Thanh toán thành công. Xe ${result.session.vehicle} đã ra cổng.`,
        );
        loadData();
        setExitInput("");
      }
  ...
  const statusConfig = {
    normal: {
      ...
      validateText: "Chưa đóng phí",
      plate: scanResult?.vehicle || "51C - 002.31",
    },
    open: {
      title: "ĐÃ XÁC THỰC - HOÀN TẤT",
      color: "bg-green-500",
      icon: "check_circle",
      bg: "bg-gradient-to-r from-teal-900/95 to-emerald-700/95",
      validateText: "Thành công ✓",
      plate: scanResult?.vehicle || "29C - 112.55",
    },
    offline: {
  ...
  const exitCount = exitLog.filter((l) => l.result === "SUCCESS").length;
  const errorCount = exitLog.filter((l) => l.result !== "SUCCESS").length;
  ...
  return (
    <div className="p-6 md:p-8 space-y-8 bg-slate-50 min-h-full font-inter relative">
      ...
      <div className="flex flex-col md:flex-row md:justify-between md:items-end gap-4 relative z-10">
        <div>
          <h2 className="text-3xl font-headline font-bold text-slate-800 tracking-tight mb-1">
            Kiểm soát Cổng ra A2
          </h2>
          <p className="text-slate-500 font-medium text-sm">
            Cơ sở Quận 10 • Xử lý giao dịch và điều phối xe ra
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-8 relative z-10 items-stretch">
        ...
        <div className="xl:col-span-7 flex flex-col gap-6 h-full">
          <div
            className={`flex-1 min-h-[400px] bg-slate-900 rounded-3xl overflow-hidden relative border border-slate-800 transition-all duration-500 ${demoState === "normal" ? "shadow-[0_20px_60px_-15px_rgba(245,158,11,0.3)]" : "shadow-xl"}`}
          >
            <div
              className={`absolute top-0 left-0 w-full px-6 py-4 ${config.bg} backdrop-blur-md flex justify-between items-center z-20 transition-colors duration-500 border-b border-white/10`}
            >
              <div className="flex items-center gap-3 text-white">
                <span className="material-symbols-outlined text-3xl opacity-90">
                  {config.icon}
                </span>
                <span className="font-headline font-bold tracking-widest text-lg drop-shadow-md">
                  {config.title}
                </span>
              </div>
              {isWaitingPayment && (
                <span className="bg-amber-400 text-amber-900 font-bold text-xs px-3 py-1.5 rounded-full uppercase tracking-wider animate-pulse">
                  Chờ thanh toán
                </span>
              )}
            </div>
            ...
            {demoState === "offline" ? (
              <div className="absolute inset-0 flex items-center justify-center flex-col text-slate-600 bg-slate-900/80 z-10">
                ...
                <p className="font-headline font-bold text-xl uppercase tracking-widest opacity-50">
              ...
              <div className="absolute inset-0 grid grid-cols-2 pt-[72px]">
                <div className="relative border-r border-slate-700">
                  <img
                    src="https://lh3.googleusercontent.com/aida-public/AB6AXuBNCp_A-NpfnkI5T-NvI09jesAIJm7BHMLBHM65gnE5cF38QLLa1piiRD6_hC_5HaryhiKfwIueba4unmp6s2pTw39E9jiHxwlcXjXhEIWxHG32ejymnCTYoPIaYkhZFhrLHB8Grc8qi4f3izLcTvpj8QKKTzz6Is3CGclQ7aEm38R0rJ1PdEN5MHIc6uq2IMVELwWIGKFmBlbWHTFDRnNBYCej0L4yWQh8bcbMb8ysU0OjIbdbwQXks7NoNGMEV7_JPR9dnJyV-KcI"
                    alt="In"
                  ...
                  <div className="absolute top-4 left-4 bg-black/60 backdrop-blur-md px-3 py-1.5 rounded-lg border-l-4 border-slate-400">
                    <span className="text-slate-300 text-[10px] font-bold block uppercase tracking-wide">
                      Ảnh lúc vào
                    </span>
                ...
                <div className="relative">
                  ...
                  <div className="absolute top-4 left-4 bg-black/60 backdrop-blur-md px-3 py-1.5 rounded-lg border-l-4 border-blue-500 flex items-center gap-2">
                    ...
                    <span className="text-white text-[10px] font-bold block uppercase tracking-wide">
                      Live Exit
                    </span>
                  </div>
                </div>
            ...
            <div className="absolute bottom-6 left-6 z-20 flex gap-4 w-[calc(100%-48px)] justify-between items-end">
              <div className="bg-slate-900/70 backdrop-blur-xl border border-white/10 p-4 rounded-2xl shadow-2xl flex gap-5 items-center">
                <div className="bg-white/95 px-5 py-2.5 rounded-xl shadow-inner border border-slate-300">
                  <span className="font-headline font-black text-3xl tracking-wider text-slate-800">
                    {scanResult?.vehicle || config.plate}
                  </span>
                </div>
              ...
              {demoState !== "offline" && (
                <div
                  className={`px-4 py-3 rounded-xl font-bold text-sm tracking-wide border backdrop-blur-md shadow-xl ${demoState === "open" ? "bg-green-500/20 border-green-400 text-green-400" : isWaitingPayment ? "bg-amber-500/20 border-amber-400 text-amber-400" : "bg-orange-500/20 border-orange-400 text-orange-400"}`}
                >
                  {isWaitingPayment
                    ? "CHỜ THANH TOÁN"
                    : "TRÙNG KHỚP ẢNH: 99.8%"}
          ...
          <div className="grid grid-cols-3 gap-4">
            {[
              ...
              [
                "Xe chờ giải quyết",
                isWaitingPayment ? "1" : "0",
                "hourglass_top",
              ],
              ["Cảnh báo/Lỗi", errorCount.toString(), "notification_important"],
            ].map(([l, v, i]) => (
              <div
                key={l}
                className="bg-white rounded-2xl p-5 border border-slate-200/80 shadow-sm flex items-center justify-between group hover:shadow-md transition-shadow"
              >
                <div>
                  <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1">
                    {l}
                  </p>
                  <p className="text-2xl font-headline font-bold text-slate-800">
                    {v}
                  </p>
                </div>
        ...
        <div className="xl:col-span-5 flex flex-col gap-6 h-full">
          {/* Scanner */}
          <div
            className={`bg-white rounded-3xl p-6 border-2 transition-colors duration-300 shadow-md ${isWaitingPayment ? "border-amber-300 bg-amber-50/30" : demoState === "normal" ? "border-orange-100 bg-orange-50/20" : "border-blue-100"}`}
          >
            <label className="text-sm font-bold text-blue-900 mb-3 flex items-center gap-2 uppercase tracking-wide">
              <span className="material-symbols-outlined text-blue-600">
                directions_car
              </span>
              Chọn xe đang trong bãi (Ra cổng)
            </label>
            <div className="relative" ref={dropdownRef}>
              <span className="material-symbols-outlined absolute left-4 top-1/2 -translate-y-1/2 text-2xl text-blue-500">
                list
              </span>

              <div
                onClick={() => setIsDropdownOpen(!isDropdownOpen)}
                className={`w-full pl-14 pr-12 py-4 rounded-xl text-lg font-bold font-mono outline-none transition-all border cursor-pointer flex items-center justify-between shadow-sm ${isWaitingPayment ? "bg-amber-50 border-amber-300 text-amber-900" : demoState === "normal" ? "bg-orange-50/50 border-orange-200 text-orange-900 hover:border-orange-400" : "bg-slate-50 text-blue-900 border-slate-200 hover:border-blue-400"}`}
              >
                {exitInput ? (
                  <span className="truncate">
                    {activeItems.find((i) => i.id === exitInput)?.label ||
                      exitInput}
                  </span>
                ) : (
                  <span className="opacity-50 font-sans font-medium text-base tracking-normal">
                    -- Bấm để chọn xe ra --
                  </span>
                )}
              </div>
              ...
              <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-4 text-slate-500">
                <span
                  className={`material-symbols-outlined transition-transform duration-300 ${isDropdownOpen ? "rotate-180 text-blue-500" : ""}`}
                >
                  expand_more
                </span>
              </div>
              ...
              {isDropdownOpen && (
                <div className="absolute top-[calc(100%+8px)] left-0 right-0 bg-white rounded-2xl shadow-[0_20px_60px_-15px_rgba(0,0,0,0.3)] border border-slate-200 z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
                  <div className="max-h-[320px] overflow-y-auto p-2 scrollbar-thin scrollbar-thumb-slate-200 scrollbar-track-transparent">
                    {activeItems.length === 0 ? (
                      <div className="p-8 text-center flex flex-col items-center justify-center gap-3">
                        ...
                        <p className="text-slate-500 font-medium">
                          Bãi đỗ hiện tại đang trống
                        </p>
                      </div>
                    ) : (
                      activeItems.map((item) => {
                        const isSelected = exitInput === item.id;
                        return (
                          <div
                            key={item.id}
                            onClick={() => {
                              setExitInput(item.id);
                              setIsDropdownOpen(false);
                            }}
                            className={`px-4 py-3 rounded-xl cursor-pointer flex items-center gap-4 transition-all border mb-1 last:mb-0 group ${isSelected ? "bg-blue-50 border-blue-200" : "hover:bg-slate-50 border-transparent hover:border-slate-200"}`}
                          >
                            <div
                              className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 transition-colors ${isSelected ? "bg-blue-600 text-white shadow-md shadow-blue-500/20" : "bg-slate-100 text-slate-500 group-hover:bg-slate-200"}`}
                            >
                              <span
                            ...
                            <div className="flex-1 min-w-0">
                              <p
                                className={`font-headline font-bold text-lg truncate mb-0.5 ${isSelected ? "text-blue-900" : "text-slate-800"}`}
                              >
                                {item.plate}
                              </p>
            ...
            <div className="mt-5 flex gap-3">
              <button
                ...
                disabled={isProcessing || !exitInput || isWaitingPayment}
                className={`flex-1 py-3.5 px-5 rounded-xl font-headline font-bold text-white transition-all text-lg flex items-center justify-center gap-2 ${isProcessing || !exitInput || isWaitingPayment ? "bg-slate-300 text-slate-500 cursor-not-allowed shadow-none" : "bg-blue-600 hover:bg-blue-700 shadow-lg shadow-blue-600/30 active:scale-95"}`}
              >
                {isProcessing ? (
                  <>
              ...
              <button
                ...
                className="py-3.5 px-6 rounded-xl border-2 border-slate-200 text-slate-600 font-bold hover:bg-slate-50 hover:text-slate-900 transition-colors active:scale-95 flex items-center justify-center"
              >
          ...
          <div className="bg-white rounded-3xl shadow-sm border border-slate-200/80 overflow-hidden flex-1 flex flex-col">
            <div className="px-6 py-4 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
              <h4 className="font-headline font-bold text-slate-800 text-lg">
                Hồ sơ Cấp phép Truy cập
              </h4>
              <span
                className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider ${demoState === "open" ? "bg-green-100 text-green-700" : isWaitingPayment ? "bg-amber-100 text-amber-700" : demoState === "offline" ? "bg-slate-100 text-slate-600" : "bg-orange-100 text-orange-700"}`}
              >
            ...
            <div className="p-6 flex flex-col gap-4 justify-center flex-1">
              <div className="flex items-center gap-4 mb-2">
                <div
                  className={`w-14 h-14 rounded-2xl flex items-center justify-center shrink-0 ${demoState === "offline" ? "bg-slate-100 text-slate-400" : isWaitingPayment ? "bg-amber-100 text-amber-700" : "bg-indigo-100 text-indigo-700"}`}
                >
                ...
                <div>
                    ...
                    {demoState === "offline"
                      ...
                      : scanResult
                        ? scanResult.cardInfo?.name ||
                          (scanResult.card?.startsWith("TEMP-")
                            ? "Khách tạm thời"
                            : "Khách vãng lai")
                        : "Chưa xác định"}
                  </p>
                  {scanResult && (
                    <p className="text-xs text-slate-500 mt-0.5">
                      {scanResult.cardInfo?.group || "Khách"}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="p-3 bg-slate-50 rounded-2xl border border-slate-100">
                  ...
                  <p className="font-bold text-slate-700 text-sm">
                    {demoState === "open" && scanResult?.exitTime
                      ? new Date(scanResult.exitTime).toLocaleTimeString(
                          "vi-VN",
                        )
                      : "---"}
                  </p>
                ...
                <div
                  className={`p-3 rounded-2xl border flex items-center justify-between ${demoState === "open" ? "bg-green-50 border-green-200 text-green-900" : isWaitingPayment ? "bg-amber-50 border-amber-200 text-amber-900 shadow-inner" : demoState === "offline" ? "bg-slate-50 border-slate-200 text-slate-400" : "bg-orange-50 border-orange-200 text-orange-900 shadow-inner"}`}
                >
                  <div>
                    <p className="text-[10px] font-bold uppercase tracking-widest mb-1 opacity-80">
                      {demoState === "open" && !isWaitingPayment
                        ? scanResult?.paymentMethod === "debt"
                          ? "GHI NỢ"
                          : "ĐÃ TT"
                        : "TỔNG PHÍ (VNĐ)"}
                    </p>
                    <p className="font-bold text-lg leading-tight font-mono">
                      {pendingFee.toLocaleString("vi-VN")}
                    </p>
                  </div>
                  {demoState === "open" && !isWaitingPayment && (
              ...
              {demoState === "open" && scanResult?.paymentMethod === "debt" && (
                <div className="p-3 bg-blue-50 border border-blue-200 rounded-xl text-blue-800 text-sm flex items-center gap-2">
                  ...
                  <span>
                    Phí{" "}
                    <strong>
                      {(scanResult?.fee || 0).toLocaleString("vi-VN")} VNĐ
                    </strong>{" "}
                    đã ghi vào công nợ thành viên.
                  </span>
                </div>
              )}
            </div>
            ...
            <div className="p-6 border-t border-slate-100 bg-slate-50">
              <div className="grid grid-cols-2 gap-4 mb-5">
                <button
                  ...
                  disabled={
                    demoState === "offline" || isBarrierOpen || isWaitingPayment
                  }
                ...
                <button
                  onClick={handleClose}
                  disabled={demoState === "offline" || !isBarrierOpen}
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-slate-200/80 overflow-hidden">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              ...
              <tbody>
                {exitLog.map((log) => {
                  const hours = log.details?.session
                    ? Math.max(
                        1,
                        Math.ceil(
                          (new Date(
                            log.details.session.exitTime || log.timestamp,
                          ) -
                            new Date(log.details.session.entryTime)) /
                            (1000 * 60 * 60),
                        ),
                      )
                    : 0;
                  ...
                  const isDebt = log.details?.paymentMethod === "debt";
                  const fee = log.details?.fee || 0;
                  ...
                  return (
                    <tr
                      key={log.id || log.timestamp + log.card}
                      className="border-b border-slate-50 hover:bg-slate-50/80 transition-colors"
                    >
                      <td className="p-4 font-mono text-slate-500 text-sm">
                        {new Date(log.timestamp).toLocaleTimeString("vi-VN")}
                      </td>
                      <td className="p-4 font-headline font-bold text-slate-800">
                        {log.plate || log.details?.vehicle || "---"}
                      </td>
                      ...
                      <td className="p-4">
                        <span
                          className={`px-3 py-1 rounded-lg text-xs font-bold ${log.details ? paymentLabel.cls : "bg-slate-100 text-slate-600"}`}
                        >
                          {log.details ? paymentLabel.label : "---"}
                        </span>
                      </td>
                      <td className="p-4">
                        <span
                          className={`px-3 py-1 rounded-lg text-xs font-bold ${resultColor}`}
                        >
                          {resStr || "SUCCESS"}
                        </span>
                      </td>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl overflow-hidden border-4 border-amber-400">
            <div className="bg-amber-50 px-8 pt-8 pb-6 text-center border-b border-amber-200">
              <span className="material-symbols-outlined text-6xl text-amber-500 mb-3 block">
                payments
              </span>
              <h3 className="text-2xl font-headline font-bold text-slate-800 mb-1">
                Chờ Thanh Toán
              </h3>
              <p className="text-slate-500 text-sm">
                {pendingExitData.type === "ticket"
                  ? `Thẻ tạm ${pendingExitData.ticket.id}`
                  : "Xe Khách vãng lai"}{" "}
                — Barrier bị khóa cho đến khi thanh toán
              </p>
            </div>
            <div className="px-8 py-6 space-y-4">
              ...
              <div className="flex items-center gap-2 p-3 bg-red-50 border border-red-200 rounded-xl text-red-700 text-sm">
                ...
                <span>
                  Barrier đang bị khóa. Xác nhận sau khi nhận đủ tiền mặt hoặc
                  quét BKPay.
                </span>
              </div>
            </div>
            <div className="px-8 pb-8 space-y-3">
              ...
              <button
                onClick={() => {
                  ...
                  setPendingExitData(null);
                  setScanResult(null);
                  setStatusMessage("Đã hủy giao dịch");
                }}
            ...
            <div className="relative bg-white w-full max-w-md rounded-3xl shadow-2xl p-8">
              ...
              <div className="space-y-4 mb-6">
                <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 text-blue-800 text-sm flex items-start gap-2">
                  ...
                  <p>
                    Nhập mã thẻ tạm (TEMP-XXXXXX) vào ô quét bên ngoài để xử lý
                    ra cổng và tính phí tự động.
                  </p>
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      if (e.target.value) setExitInput(e.target.value);
                    }}
                  />
                </div>
              </div>
