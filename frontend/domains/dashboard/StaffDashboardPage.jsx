...
export default function StaffDashboardPage() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        const [staffStats, currentAlerts] = await Promise.all([
  ...
  const KPI = [
    { label: 'Xe đang đỗ', value: stats.activeVehicles.toLocaleString(), sub: 'chiếc', icon: 'directions_car', trend: 'Hoạt động', trendUp: true, extra: `Hiệu suất bãi: ${stats.occupancyRate}%` },
    { label: 'Cổng Vào hôm nay', value: stats.sessionsToday.toLocaleString(), sub: 'lượt', icon: 'login', trend: 'Thực tế', trendUp: true, extra: 'Trực tiếp từ cảm biến' },
    { label: 'Trống hiện tại', value: stats.availableSlots.toLocaleString(), sub: 'chỗ', icon: 'local_parking', trend: 'Còn trống', trendUp: true, extra: `Tổng cộng ${stats.totalSlots} chỗ` },
  ]
  ...
  const ALERTS = alerts.length > 0 && alerts[0].title ? alerts : [
    { bg: '#fff1f2', border: '#fecdd3', icon: 'sensors_off', ic: '#ef4444', title: 'Cảm biến mất tín hiệu', badge: 'Nghiêm trọng', bc: '#fce7f3', bct: '#be185d', desc: 'Cảm biến không phản hồi. Cần kiểm tra ngay.', time: 'Vừa xong', btn: 'Kiểm tra' },
    ...
    <div style={{ padding: 32, display: 'flex', flexDirection: 'column', gap: 28 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: 12 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: '#fff', padding: '8px 16px', borderRadius: 10, border: '1px solid #e2e8f0', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
          ...
          <span style={{ fontSize: 13, fontWeight: 600 }}>Trực tuyến ({stats.onlineDevices}/{stats.totalDevices} thiết bị)</span>
        </div>
      </div>
